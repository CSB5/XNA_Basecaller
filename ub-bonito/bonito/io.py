...
class CTCWriter(Thread):
    ...
    def __init__(
            ...
            min_accuracy=0.95, ref_fn=None, groups=None, group_key=None, ub_only=False,
    ):
        super().__init__()
        self.fd = fd
        self.log = []
    ...
    def run(self):
        ...
        with CSVLogger(summary_file(), sep='\t') as summary:
            count_failed_both = count_failed_cov = count_failed_acc = 0
            count_failed_seq = count_failed_map = 0
            non_ubs_skipped = 0
            for read, ctc_data in self.iterator:
                ...
                #### [my-mod] For only saving chunks containing 'N' in reference
                if self.ub_only and 'N' not in refseq:
                    non_ubs_skipped += 1
                    continue
                ...
                if cov < self.min_coverage:
                    count_failed_cov += 1
                if acc < self.min_accuracy and cov < self.min_coverage:
                    count_failed_both += 1
                
                #### [my-mod] Allowing chunks containing 'N' in reference
                #if acc < self.min_accuracy or cov < self.min_coverage or 'N' in refseq:
                if acc < self.min_accuracy or cov < self.min_coverage:
                    continue
                ...
                #target = [int(x) for x in refseq.translate({78: '0', 65: '1', 67: '2', 71: '3', 84: '4'})]
                
                #### [my-mod] Replacing N by X/Y
                ub_target = '5' if mapping.strand != -1 else '6'
                target = [int(x) for x in refseq.translate({65: '1', 67: '2', 71: '3', 84: '4', 78: ub_target})]
        ...
        if self.ub_only:
            sys.stderr.write("Non-UB chunks skipped: {:0,d}\n".format(non_ubs_skipped))
        
        sys.stderr.write("Filtered reads (failed): {:0,d} seq, {:0,d} map\n".format(
            count_failed_seq, count_failed_map))
        sys.stderr.write("Filtered reads (failed): {:0,d} acc, {:0,d} cov, {:0,d} both\n".format(
            count_failed_acc, count_failed_cov, count_failed_both))
        filter_ser = pd.Series(dict(
            count_failed_seq=count_failed_seq, count_failed_map=count_failed_map,
            count_failed_acc=count_failed_acc, count_failed_cov=count_failed_cov, 
            count_failed_both=count_failed_both, non_ubs_skipped=non_ubs_skipped))
        ...
        filter_ser.to_csv(os.path.join(output_directory, "filter_stats.csv"))
