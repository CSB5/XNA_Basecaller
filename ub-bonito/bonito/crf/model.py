...
def rnn_encoder(n_base, state_len, insize=1, stride=5, winlen=19, activation='swish', 
                rnn_type='lstm', features=768, scale=5.0, blank_score=None, expand_blanks=True,
                extra_linear=False, drop_rate=0, drop_rate_bottom=0):
    rnn = layers[rnn_type]
    if drop_rate_bottom == 0:
        return Serial([
                ...
                LinearCRFEncoder(
                    ...
                    extra_linear=extra_linear, drop_rate=drop_rate, # My addition
                )
        ])
    else:
        # dropout = torch.nn.Dropout(p=drop_rate_bottom)
        # return Serial([
        #         conv(insize, 4, ks=5, bias=True, activation=activation), dropout,
        #         conv(4, 16, ks=5, bias=True, activation=activation), dropout,
        #         conv(16, features, ks=winlen, stride=stride, bias=True, activation=activation), dropout,
        #         Permute([2, 0, 1]),
        #         # rnn(features, features, reverse=True), rnn(features, features), dropout,
        #         # rnn(features, features, reverse=True), rnn(features, features), dropout,
        #         rnn(features, features, reverse=True), dropout,
        #         rnn(features, features), dropout,
        #         rnn(features, features, reverse=True), dropout,
        #         rnn(features, features), dropout,
                    
        #         rnn(features, features, reverse=True), # dropout already present at LinearCRFEncoder 
        #         LinearCRFEncoder(
        #             features, n_base, state_len, activation='tanh', scale=scale,
        #             blank_score=blank_score, expand_blanks=expand_blanks, 
        #             extra_linear=extra_linear, drop_rate=drop_rate, # My addition
        #         )
        # ])
        ### Changing each dropout to each own object, to independently change it to eval() afterwards
        return Serial([
                conv(insize, 4, ks=5, bias=True, activation=activation), torch.nn.Dropout(p=drop_rate_bottom),
                conv(4, 16, ks=5, bias=True, activation=activation), torch.nn.Dropout(p=drop_rate_bottom),
                conv(16, features, ks=winlen, stride=stride, bias=True, activation=activation), torch.nn.Dropout(p=drop_rate_bottom),
                Permute([2, 0, 1]),
                # rnn(features, features, reverse=True), rnn(features, features), torch.nn.Dropout(p=drop_rate_bottom),
                # rnn(features, features, reverse=True), rnn(features, features), torch.nn.Dropout(p=drop_rate_bottom),
                rnn(features, features, reverse=True), torch.nn.Dropout(p=drop_rate_bottom),
                rnn(features, features), torch.nn.Dropout(p=drop_rate_bottom),
                rnn(features, features, reverse=True), torch.nn.Dropout(p=drop_rate_bottom),
                rnn(features, features), torch.nn.Dropout(p=drop_rate_bottom),
...
class Model(SeqdistModel):
    ...
    def __init__(self, config):
        ...
        if 'type' in config['encoder']: #new-style config
            ...
            encoder = rnn_encoder(seqdist.n_base, seqdist.state_len, 
