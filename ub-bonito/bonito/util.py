...
def match_names(state_dict, model, skip_layers=[]):
    ...
    # sys.stderr.write(f"> s1: {s1}\n")
    # sys.stderr.write(f"> s2: {s2}\n")
    # sys.stderr.write(f"> k1: {k1}\n")
    # sys.stderr.write(f"> k2: {k2}\n")
    assert s1 == s2
...
def load_model(dirname, device, weights=None, half=None, chunksize=None, batchsize=None, 
               overlap=None, quantize=False, use_koi=False, skip_top=False, drop_rate=None,
               drop_rate_bottom=None):
    """
    ...
    # sys.stderr.write(f"* model weights: {weights}\n")
    ...
    overlap = basecall_params["overlap"] = overlap if overlap is not None else basecall_params.get("overlap", 500)
    ...
    config["encoder"]['drop_rate'] = drop_rate if drop_rate is not None else config["encoder"].get("drop_rate", 0)
    config["encoder"]['drop_rate_bottom'] = drop_rate_bottom if drop_rate_bottom is not None else config["encoder"].get("drop_rate_bottom", 0)

    Model = load_symbol(config, "Model")
    model = Model(config)
    
    #### Setting use_koi to False when num of bases bigger than 4
    if model.seqdist.n_base != 4 and use_koi:
        sys.stderr.write("[Warning] Setting use_koi to False because n_base != 4.\n")
        use_koi = False
        # sys.stderr.write("[WARNING] NOT Setting use_koi to False, even though n_base != 4.\n")
    
    if use_koi:
        # sys.stderr.write("Before use_koi -> koi.lstm.update_graph\n")
        # sys.stderr.write(f"Last layer {model.encoder[-1]}\n")
        # sys.stderr.write(f"blank_score {model.encoder[-1].blank_score}\n")
        # sys.stderr.write(f"expand_blanks {model.encoder[-1].expand_blanks}\n")
        
        model.encoder = koi.lstm.update_graph(
            model.encoder, batchsize=batchsize, chunksize=chunksize // model.stride, quantize=quantize,
            # expand_blanks=True # update_graph() got an unexpected keyword argument 'expand_blanks'
        )

    # sys.stderr.write("[Warning] Forcing expand_blanks to be True\n")
    # model.encoder[-1].expand_blanks = True
    
    # sys.stderr.write("After use_koi -> koi.lstm.update_graph\n")
    # sys.stderr.write(f"Last layer {model.encoder[-1]}\n")
    # sys.stderr.write(f"blank_score {model.encoder[-1].blank_score}\n")
    # sys.stderr.write(f"expand_blanks {model.encoder[-1].expand_blanks}\n")
    # sys.stderr.write(f"model.stride {model.stride}\n")
    
    state_dict = torch.load(weights, map_location=device)
    # sys.stderr.write(f"> weights: {weights}\n")
    # sys.stderr.write(f"> state_dict.keys(): {state_dict.keys()}\n")
    # sys.stderr.write(f"> model.state_dict().keys(): {model.state_dict().keys()}\n")
    
    #### [ad-hoc] skip_top
    if skip_top:
        sys.stderr.write("[WARNING: skipping top layer weights]\n")
        ## remove top layer (LinearCRFEncoder) from state_dict
        #### TODO add dynamic layer name finding? sort by id then get last x.weight/bias
        # Need to add new linear_ext (either manually or get all from LinearCRFEncoder)
        # skip_layers = ['encoder.9.linear.weight', 'encoder.9.linear.bias']
        skip_layers = [ layer_name for layer_name in model.state_dict().keys()
                        if layer_name.startswith('encoder.9') ]
    #     # top_layer = 'encoder.9'
    #     del state_dict['encoder.9.linear.weight']
    #     del state_dict['encoder.9.linear.bias']
        sys.stderr.write(f"> Load weights skip layers: {skip_layers}\n")
    else:
        skip_layers = []
    
    # state_dict = {k2: state_dict[k1] for k1, k2 in match_names(state_dict, model).items()}
    state_dict = {k2: state_dict[k1] 
                  for k1, k2 in match_names(state_dict, model, skip_layers).items()}
    ...
    missing_keys , unexpected_keys = model.load_state_dict(new_state_dict, strict=not skip_top)
    assert unexpected_keys == []
    assert missing_keys == skip_layers
    # sys.stderr.write(f"> missing_keys   : {missing_keys}\n")
    # sys.stderr.write(f"> unexpected_keys: {unexpected_keys}\n")
