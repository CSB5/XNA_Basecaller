...
class LinearCRFEncoder(Module):
    ...
    def __init__(self, insize, n_base, state_len, bias=True, scale=None, activation=None, 
                 blank_score=None, expand_blanks=True, extra_linear=False, drop_rate=0):
        super().__init__()
        self.scale = scale
        self.n_base = n_base
        ...
        #### Adding another linear (FC) layer between LSTM and top Linear
        self.extra_linear = extra_linear
        if extra_linear:
            self.linear_ext = torch.nn.Linear(insize, insize, bias=bias)
            # import sys
            # sys.stderr.write(f"* Dropout rate: {drop_rate}\n")
    ...
    def forward(self, x):
        if self.extra_linear:
            # x = self.dropout(self.linear_ext(x))
            x = self.linear_ext(x)
        x = self.dropout(x)
        ...
        if self.blank_score is not None and self.expand_blanks:
            #### NOTE here is where log_probs is generated and expanded
            T, N, C = scores.shape
        ...
        # import sys
        # sys.stderr.write(f">>> {self.blank_score=}\n")
        # sys.stderr.write(f">>> {self.expand_blanks=}\n")
        return scores
    ...
    def to_dict(self, include_weights=False):
        ...
        if include_weights:
            ...
            if self.extra_linear:
                res['params']['W_ext'] = self.linear_ext.weight
                res['params']['b_ext'] = self.linear_ext.weight if self.linear_ext.bias is not None else []
        return res
